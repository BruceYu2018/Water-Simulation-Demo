...
public class InteractiveWaterCPU : MonoBehaviour {
    ...
    //Splash
    public static List<Vector3> splashRegion = new List<Vector3>();
    ...
    void AddWaterWakes(float dt)
    {
        ...
        for (int j = 0; j < arrayLength; j++)
        {
            for (int i = 0; i < arrayLength; i++)
            {
                ...
                //Store the new height if it is greater than a threshold
                if (newHeight > 0.05f)
                {
                    splashRegion.Add(new Vector3(i*gridSpacing-waterWidth/2, newHeight, j*gridSpacing-waterWidth/2));
                }
            }
        }
    }
