Shader "demoShaders/InteractiveWaveUpdate"
{
	Properties
	{
	...
	SubShader
	{
        ...
        float4 updateHeight(float4 currentHeight, float4 previousHeight, float verticalDerivative, float4 data)
        {
            ...
            float numerator1  = 2 - _alpha * dt;
            float denominator = 1 + _alpha * dt;
            float numerator2 = g * dt * dt;
            float height = (currentHeight.y + source)*obstruction;
            
            newHeight += height * numerator1;
            newHeight -= previousHeight.y;
            newHeight -= numerator2 * verticalDerivative;
		...
		f2o update (v2f i)
		{
            ...
            float4 currentNormal = float4(calculateNormal(i.uv), 1);
